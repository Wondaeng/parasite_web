...
def get_dataset(name, image_set, transform, data_path, num_classes=2):
...
def get_args_parser(add_help=True):
    ...
    parser.add_argument("--checkpoint", default="", type=str, help="path of checkpoint for evaluation")
...
def main(args):
    ...
    if utils.is_main_process():
        # passing argparse config with hyper-parameters
        tensorboard_.args = vars(args)
    # ### end: for tensorboard ###
    ...
    # esla added for checkpoint models
    ...
    # ### start: esla added to save the best epoch ###
    best_mAP = 0.40
    # ### end: esla added to save the best epoch ###
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        lr_scheduler.step()
        # esla commented out: moved it to after evaluation
        # if args.output_dir:
        #     checkpoint = {
        ...
        coco_results = evaluate(model, data_loader_test, device=device)
        ...
        if args.output_dir:
            ...
            # utils.save_on_master(checkpoint, os.path.join(args.output_dir, f"model_{epoch}.pth"))
            # utils.save_on_master(checkpoint, os.path.join(args.output_dir, "checkpoint.pth"))
            curr_mAP = coco_results.coco_eval['bbox'].stats[0]
            if curr_mAP > best_mAP:
                print("*"*40)
                print(f"Saving ... The last best was {best_mAP}")
                print("*" * 40)
                best_mAP = curr_mAP
